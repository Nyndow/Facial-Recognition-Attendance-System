...
yolo_model: YOLO | None = None   # Loaded once per session


#  Shared-frame helpers  (read by main thread)
...
def clahe_enhance(img: np.ndarray) -> np.ndarray:
    """Improve brightness / contrast with CLAHE in LAB colour space."""
...
def run_detection(
    session_id,
    args,
    embedding_list: list[np.ndarray] | None = None,
    ...
    while _running:
        ...
        if frame_idx % args.skip_frames == 0:
            ...
            for idx, ((x1, y1, x2, y2), _score) in enumerate(last_stable):
                ...
                if slot["count"] >= args.confirm_frames:
                    ...
                    if not registry.is_known(avg_emb):
        ...
        _set_latest_frame(annotated)   # ← replaces cv2.imshow
    ...
    # Clear the shared frame so the display loop shows nothing when idle
